...
## 16.可选和序列化
如上所述，Optional用作返回类型。不建议尝试将其用作字段类型。

此外，在可序列化的类中使用Optional将导致NotSerializableException。我们的文章Java Optional作为Return Type进一步解决了序列化的问题。

并且，在“与杰克逊一起使用可选”中，我们说明了对“可选”字段进行序列化时会发生的情况，以及一些实现所需结果的变通办法。
